...
class News_DB:
    ...
    def query_urls(self, client_name, source_name):
        try:
            ...
            items = list(container.query_items(
                query="SELECT r.news_url FROM r WHERE r.client_name = @client_name and r.source_name = @source_name",
                parameters=[
                    {"name": "@client_name", "value": client_name},
                    {"name": "@source_name", "value": source_name}
                ],
